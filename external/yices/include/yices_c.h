...
/**
   \brief Struct used by yices_get_arith_value_as_string and yices_free_string.

   It consists of an integer flag + a pointer
   - if the flag is 0, the pointer is NULL
   - if the flag is 1, the pointer is a valid '0' terminated C-string
*/
...
/**
   \brief Check whether the logical context is known to be inconsistent.
   ...
   is unknown. To determine the status in this case, call #yices_check.
*/
...
/**
  \brief Convert the value assigned to a  variable \c v in model \c m to a 
  character string. The string is returned into a record of the previous type.

   If the value of v can't be returned, then 
     r->flag is set to 0
     r->str is set to NULL
   otherwise
     r->flag is set to 1
     r->str = v's value converted to a string (decimal format).

   The string r->str is allocated internally by yices. To free it 
   when it's no longer needed, call yices_free_string(r).
*/
void yices_get_arith_value_as_string(yices_model m, yices_var_decl d, srec_t *r);


/**
  \brief Free the string returned by the preceding function in r.
  This also resets r->flag to 0 and r->str to NULL.
*/
...
/**
   \brief Get the value assigned to a variable of scalar type in model m as an
   integer index

   If <tt>d</tt> is a variable declaration of a scalar type <tt>tau</tt>, then 
   <tt>tau</tt> must have been defined as (define-type tau (scalar e_0 ... e_{n-1}))
   for some <tt>n>0</tt>. 
   ...
   Possible errors are:
   - \c d is not the declaration of a scalar variable
   - \c d is not assigned a value in model m
*/
...
/**
   \brief Get the value assigned to a variable of scalar type in model m as a string

   If <tt>d</tt> is a variable declaration of a scalar type <tt>tau</tt>, then 
   <tt>tau</tt> must have been defined as (define-type tau (scalar e_0 ... e_{n-1}))
   for some <tt>n>0</tt>. 

   This function returns the value of variable d in model m as one of the strings "e_{i}"
...
/**
  \brief Parse string s as a Yices expression and return the expression.
  The string must use the Yices input language (not SMT-LIB).

  \return NULL if there's an error in parsing or typechecking.
  \return the expression otherwise.

  If there's an error, function yices_get_last_error_message can be used
...
/**
  \brief Parse string s as a Yices type and return the type.
  The string must uses the Yices input language (not SMT-LIB).

  \return NULL if there's an error in parsing or typechecking.
  \return the type otherwise.
...
/**
   \brief Parse and execute the command in string s. 
   s must use the Yices input language.

  \return 1 if the command executes without error.
  \return 0 otherwise.
...
/**
   \brief Build a function update term <tt>(update f (t1 ... tn) v)</tt>.
...
/**
   \brief Build the tuple term <tt>(tuple t1 ... tn)</tt>.
*/
yices_expr yices_mk_tuple_literal(yices_context ctx, yices_expr arg[], unsigned n);
