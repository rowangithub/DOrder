...
type annotation =
  ...
  | An_call of Location.t * Env.call
  | An_ident of Location.t * string * Env.annoident
;;
...
let print_ident_annot pp str k =
  match k with
  | Env.Idef l -> fprintf pp "def %s %a@." str print_location l;
  | Env.Iref_internal l -> fprintf pp "int_ref %s %a@." str print_location l;
  | Env.Iref_external -> fprintf pp "ext_ref %s@." str;
;;
