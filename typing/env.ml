...
type t = {
  ...
  annotations: (Path.t * annoident) Ident.tbl;
...
and structure_components = {
  ...
  mutable comp_annotations: (string, (annoident * int)) Tbl.t;
        ...
        match item with
          Tsig_value(id, decl) ->
            ...
            if !Clflags.annotations then begin
              c.comp_annotations <-
                Tbl.add (Ident.name id) (Iref_external, !pos)
                        c.comp_annotations;
            end;
      ...
      (fun env item p ->
        match item with
          Tsig_value(id, decl) ->
            ...
            in store_annot (Ident.hide id) p (Iref_external) e1
        | Tsig_type(id, decl, _) ->
