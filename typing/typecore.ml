...
let pattern_scope = ref (None : Env.annoident option);;
...
let add_pattern_variables env =
  ...
  List.fold_right
    (fun (id, ty, loc) env ->
       ...
       Env.add_annot id (Env.Iref_internal loc) e1;
    )
    pv env
  ...
  | Tmod_structure items ->
      List.for_all
        (function
          ...
          | Tstr_open _ | Tstr_cltype _ | Tstr_exn_rebind _ | Tstr_qualifier _ -> true
...
let rec type_exp env sexp =
  ...
  | Pexp_let(rec_flag, spat_sexp_list, sbody) ->
      let scp =
        match rec_flag with
        | Recursive -> Some (Env.Idef loc)
        | Nonrecursive -> Some (Env.Idef sbody.pexp_loc)
        | Default -> None
      in
  ...
  let pat_env_list =
    List.map
      (fun (spat, sexp) ->
        let loc = sexp.pexp_loc in
        if !Clflags.principal then begin_def ();
        let scope = Some (Env.Idef loc) in
