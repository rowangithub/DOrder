...
let tostring loc = 
	let (_, line, startchar) = get_pos_info loc.loc_start in
  let endchar = loc.loc_end.pos_cnum - loc.loc_start.pos_cnum + startchar in
  let (startchar, endchar) =
    if startchar < 0 then (0, 1) else (startchar, endchar) in
	((string_of_int line) ^ "_" ^
		(string_of_int startchar) ^ "_" ^ (string_of_int endchar))
