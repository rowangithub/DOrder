...
let view_fixity_of_exp = function
  | {pexp_desc = Pexp_ident (Lident l); _} -> fixity_of_string l
  | _ -> `Normal  ;;
...
let view_expr x =
  ...
  | Pexp_construct ( (Lident "()"),_,_) -> `tuple
  | Pexp_construct ( (Lident "[]"),_,_) -> `nil
  | Pexp_construct ( (Lident"::"),Some _,_) ->
      let rec loop exp acc = match exp with
          | {pexp_desc=Pexp_construct ((Lident "[]"),_,_);_} -> (List.rev acc,true)
          | {pexp_desc=
             Pexp_construct ((Lident "::"),Some ({pexp_desc= Pexp_tuple([e1;e2]);_}),_);_} ->
              loop e2 (e1::acc)
  ...
  | Pexp_construct (x,None,_) -> `simple (x)
  | _ -> `normal
...
class printer fname env lightenv lookup_fr se_env udt_table = object(self:'self)
  val pipe = false
  val semi = false
  val ifthenelse = false
	val env = env
	val lightenv = lightenv
	val lookup_fr = lookup_fr
	val se_env = se_env
	val fname = fname
	val udt_table = udt_table
  ...
  method longident_loc f x = pp f "%a" self#longident x
  ...
  method type_var_option f str = self#tyvar f str
    (*match str with
    | None -> pp f "_"
    | Some txt -> self#tyvar f txt *)
  ...
  method core_type1 f x =
    ...
    | Ptyp_package (lid, cstrs) ->
        let aux f (s, ct) =
          pp f "type %s@ =@ %a" s self#core_type ct  in
        (match cstrs with
  ...
  method pattern f x =
    ...
    | Ppat_alias (p, s) -> pp f "@[<2>%a@;as@;%a@]"
          self#pattern p
          (fun f s->
            if is_infix (fixity_of_string s) || List.mem s.[0] prefix_symbols
            then pp f "( %s )" s
            else pp f "%s" s ) s (* RA*)
    | Ppat_or (p1, p2) -> (* *)
        (match p1 with
  ...
  method pattern1 (f:Format.formatter) (x:pattern) :unit =
    ...
    | Ppat_construct ((Lident("()"|"[]")), _, _) -> self#simple_pattern f x
    | Ppat_construct (li, po, _) -> (* FIXME The third field always false *)
        if li = Lident "::" then
  ...
  method simple_pattern (f:Format.formatter) (x:pattern) :unit =
    match x.ppat_desc with
    | Ppat_construct ((Lident ("()"|"[]" as x)), _, _) -> pp f  "%s" x
    | Ppat_any -> pp f "_";
    | Ppat_var (txt) ->
    ...
    | Ppat_record (l, closed) ->
        let longident_x_pattern f (li, p) =
          ...
          | ((Lident s),Ppat_var txt) when s = txt ->
  ...
  method sugar_expr f e =
    ...
    | Pexp_apply
        ({pexp_desc=
          Pexp_ident
            (Ldot (Lident (("Array"|"String") as s),"get"));_},
         [(_,e1);(_,e2)]) -> begin
							if (s = "Array") then
								pp f "@[%a.(%a)@]" self#simple_expr e1 self#expression e2
							else
								pp f "@[%a.[%a]@]" self#simple_expr e1 self#expression e2
              ;
              true
    ...
    |Pexp_apply
        ({pexp_desc=
          Pexp_ident
            (Ldot (Lident (("Array"|"String") as s),
                        "set"));_},[(_,e1);(_,e2);(_,e3)])
      ->
        if s = "Array" then
					pp f "@[%a.(%a)@ <-@;%a@]" self#simple_expr e1 self#expression e2 self#expression e3
				else 
					pp f "@[%a.[%a]@ <-@;%a@]" self#simple_expr e1 self#expression e2 self#expression e3
				;
        true
    | Pexp_apply ({pexp_desc=Pexp_ident (Lident "!");_}, [(_,e)]) -> begin
        pp f "@[<hov>!%a@]" self#simple_expr e;
        true
    end
    | Pexp_apply
        ({pexp_desc=Pexp_ident
                     (Ldot (Ldot (Lident "Bigarray", array), ("get"|"set" as gs)) );_},
         label_exprs) ->
  ...
  method expression f x =
    ...
    | Pexp_apply (e, l) ->
        (if not (self#sugar_expr f x) then
          ...
          | _ ->
						(Instrument.dump_app f se_env x.pexp_loc x;
    ...
    | Pexp_setinstvar (s, e) ->
        pp f "@[<hov2>%s@ <-@ %a@]" s self#expression e
    | Pexp_override l -> (* FIXME *)
        let string_x_expression f (s, e) =
          pp f "@[<hov2>%s@ =@ %a@]" s self#expression e in
        pp f "@[<hov2>{<%a>}@]"
          (self#list string_x_expression  ~sep:";"  )  l;
    | Pexp_letmodule (s, me, e) ->
        pp f "@[<hov2>let@ module@ %s@ =@ %a@ in@ %a@]" s
  ...
  method simple_expr f x =
    ...
    | Pexp_for (s, e1, e2, df, e3) ->
        ...
        pp f fmt s self#expression e1 self#direction_flag df self#expression e2  self#expression e3
  ...
  method class_type_declaration_list f  l =
    let class_type_declaration f ({pci_params=(ls,_);pci_name=txt;pci_variance;_} as x) =
  ...
  method class_field f x =
    ...
    | Pcf_meth (s, pf, ovf, e, _) ->
        pp f "@[<2>method%s %a%a@]"
          ...
          | Pexp_poly (e, Some ct) ->
              pp f "%s :@;%a=@;%a"
                s (self#core_type) ct self#expression e
          | Pexp_poly (e,None) ->
    ...
    | Pcf_cstr (ct1, ct2, _) ->
		...
		| Pcf_let _ -> 
			(** FixME. Unsupported *) (print_string "Pcf_let unsupported"; assert false)
    | Pcf_init (e) ->
        pp f "@[<2>initializer@ %a@]" self#expression e

  method class_structure f (p, l) =
    ...
    | Psig_value (s, vd) ->
        pp f "@[<2>%a@]"
          (fun f (s,vd) ->
            ...
            if (is_infix (fixity_of_string s)) || List.mem s.[0] prefix_symbols then
              pp f "%s@ (@ %s@ )@ :@ " intro s
            else
              pp f "%s@ %s@ :@ " intro s;
            self#value_description f vd;) (s,vd)
    | Psig_exception (s, ed) ->
        self#exception_declaration f (s,ed)
    | Psig_class l ->
        let class_description f ({pci_params=(ls,_);pci_name=txt;pci_variance;_} as x) =
  ...
  method module_expr f x =
    ...
    | Pmod_functor (s, mt, me) ->
        pp f "functor@ (%s@ :@ %a)@;->@;%a"
          s  self#module_type mt  self#module_expr me
    | Pmod_apply (me1, me2) ->
        pp f "%a(%a)" self#module_expr me1  self#module_expr  me2
    | Pmod_unpack (e, _) ->
        pp f "(val@ %a)"  self#expression  e

  method structure f x = (
		pp f "@[<2>open@;%s@]" "Printf";
		pp f "@;@[<hov2>let@ xxxcounter =@ ref 1000 @]@.";
		pp f "@;@[<hov2>let@ callflag =@ ref true @]@.";
		pp f "@;@[<hov2>let@ outch =@ open_out \"%s\" @]@." Instrument.log_fname;
		Instrument.print_min_max f;
		self#list ~sep:"@\n" self#structure_item f x;
		Instrument.print_harnesses fname f;
		pp f "@;@[<hov2>let@ _ =@ close_out outch@;@]"
	)

  (* transform [f = fun g h -> ..] to [f g h = ... ] could be improved *)
  method binding f ((p:pattern),(x:expression)) =
    let xloc = x.pexp_loc in
		let xxx = x in
		let rec pp_print_pexp_function functionflag f x =
      match x.pexp_desc with
      | Pexp_function (label,eo,[(p,e)]) ->
          if label="" then
            ...
            | _ ->
                pp f "%a@ %a" self#simple_pattern p (pp_print_pexp_function true) e
          else
            pp f "%a@ %a" self#label_exp (label,eo,p) (pp_print_pexp_function true) e
      | Pexp_newtype (str,e) ->
          pp f "(type@ %s)@ %a" str (pp_print_pexp_function functionflag) e
      | _ -> 
				if (functionflag) then
					(** dump function parameters and returns *)
					(*pp f "=@;@[<2>let r = @;%a in@;let _ = %s in r@]" 
					self#expression x ""*)
					((*pp f "=@;@[<2>let r = @;%a@;@]" *)
					
					match p.ppat_desc with
						| Ppat_var txt -> 
			 				(pp f "="; (Instrument.dump_side_effects f se_env lookup_fr xloc txt env lightenv);
							pp f "@;@[<2>let r = %a@;@]"
							self#expression x;
							Instrument.dump_fun f se_env lookup_fr xloc txt env lightenv xxx)
						| _ -> (print_string "function cannot be dumped\n"; assert false))
				else	
					pp f "=@;%a" self#expression x in
    ...
    | ( _ , Ppat_constraint( p ,ty)) -> (* special case for the first*)
        ...
        | _ ->
            pp f "(%a@;:%a)=@;%a" self#simple_pattern p  self#core_type ty self#expression x)
    | Pexp_constraint (e,Some t1,None),Ppat_var txt ->
        pp f "%s:@ %a@;=@;%a" txt self#core_type t1  self#expression e
    | (_, Ppat_var _) ->
        pp f "%a@ %a" self#simple_pattern p (pp_print_pexp_function false) x
    | _ ->
  ...
  method structure_item f x = begin
    ...
    | Pstr_type l  -> (self#type_def_list f l; Instrument.print_udt (List.map fst l) udt_table f)
    | Pstr_value (rf, l) -> (* pp f "@[<hov2>let %a%a@]"  self#rec_flag rf self#bindings l *)
        pp f "@[<2>%a@]" self#bindings (rf,l)
    | Pstr_exception (s, ed) -> self#exception_declaration f (s,ed)
    | Pstr_module (s, me) ->
        let rec module_helper me = match me.pmod_desc with
        | Pmod_functor(s,mt,me) ->
            pp f "(%s:%a)"  s  self#module_type mt ;
            module_helper me
        | _ -> me in
    ...
    | Pstr_modtype (s, mt) ->
        pp f "@[<2>module type %s =@;%a@]" s self#module_type mt
    | Pstr_class l ->
  ...
  method type_declaration f x = begin
    let  type_variant_leaf f  (s, l, _loc)  = 
			pp f "@\n|@;%s%a" s
          (fun f l -> match l with
          | [] -> ()
    ...
    pp f "%a%a@ %a"
      ...
      | Ptype_record l ->
          let type_record_field f (s, mf, ct,_) =
            pp f "@[<2>%a%s:@;%a@]" self#mutable_flag mf s self#core_type ct in
          pp f "{@\n%a}"
